...
reap_stray "$OXR_HELPER_BIN" \
  "encoder helper killed (the runtime owns it — this one outlived its game)" \
  "no leftover encoder helper"

reap_stray "$ALVR_DASHBOARD_BIN" "ALVR dashboard closed (left over from a run that died uncleanly)"
