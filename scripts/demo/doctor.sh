...
else fail "rustup x86_64-apple-darwin target missing" "rustup toolchain install stable && rustup target add x86_64-apple-darwin"; fi
...
# 8. Beat Saber 1.29.4
if [ -f "$BS_DIR/Beat Saber.exe" ]; then
  BSVER="$(bs_version)"
  case "$BSVER" in
