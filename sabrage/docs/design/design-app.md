...
- **Dev caveat:** `cargo tauri dev` runs an unbundled binary — TCC treats it separately.
- **Spike result (2026-08-29, Phase 2):** the machine already holds a stable Apple-issued identity (`Apple Development: Yifei Ding (NNRXG97EW5)`), so the self-signed "Sabrage Dev" cert is unnecessary. Measured with `codesign -d -r-` on `tauri build --debug --bundles app`: the identity-signed designated requirement is `identifier "dev.dingyifei.sabrage" and anchor apple generic and certificate leaf[subject.CN] = "Apple Development: …" and certificate 1[field.1.2.840.113635.100.6.2.1]` — no cdhash, byte-identical across rebuilds — whereas an ad-hoc build (`APPLE_SIGNING_IDENTITY=-`) carries no authority chain and a cdhash-bound requirement. TCC keys off the designated requirement, so the App Management grant survives rebuilds under the identity. Decision: keep `bundle.macOS.signingIdentity` **out of** `tauri.conf.json` (nothing personal committed) and sign at build time via `APPLE_SIGNING_IDENTITY="Apple Development: Yifei Ding (NNRXG97EW5)" npm run tauri build`; `spctl -a` reports "rejected" (Apple Development is not Gatekeeper-distributable) but a locally built bundle has no quarantine xattr, so Finder launch is unaffected. Until the Settings "Repository" card lands (Phase 4), launch the bundle from inside the checkout (`sabrage/target/*/bundle/macos/Sabrage.app`) so the exe-walk-up finds the repo root; the grant follows the bundle id + signature, not the path. Test TCC-touching flows against `cargo tauri build` bundles; day-to-day dev works fine because everything else is unprivileged.
