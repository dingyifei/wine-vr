...
| **Config readers use the runtime's semantics.** `cfg.protocol.*`, the run preflight's protocol/encoder facts and the helper fix read `oxrsys-runtime.toml` the way the runtime does (last assignment wins, table-blind, quote-aware `#`); both shells now use last-match `awk` (`{v=$2} END{print v}`), so the only remaining difference is that the native reader skips an *invalid* last value in favour of the previous valid one, exactly as `Config.cpp` does, where `awk` reports the invalid last value. | The old first-match readers could validate a value the runtime ignored (a shadowed earlier line). Tap verdicts agree whenever the last assignment is valid; an invalid trailing assignment is a FAIL on the shell side and the runtime's effective value natively. |
