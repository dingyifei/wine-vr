...
mod slug_coverage {
    ...
    pub(super) fn scan_doctor_slugs(text: &str) -> Scan {
        let chk_re = Regex::new(r"\bchk\s+(?:ok|warn|fail|info)\s+([a-z][a-z0-9._-]*)\b").unwrap();
        let tap_re =
            Regex::new(r"\btap\s+([a-z][a-z0-9._-]*)\s+(?:ok|warn|fail|info|skipped)\b").unwrap();
...
mod run_sh_tags {
    ...
    fn tag_groups(text: &str) -> Vec<TagGroup> {
        let tag_re =
            Regex::new(r"^#\s*(preflight:|preflight-warn:|preflight-autofix:)\s+(.+)$").unwrap();
...
mod artifact_goldens {
    ...
    fn toml_template_matches_the_on_disk_contract_file() {
        let root = repo_root();
        let on_disk = std::fs::read_to_string(root.join("contract/oxrsys-runtime.toml.template"))
...
mod launch_goldens {
    ...
    fn wine_spec_argv_matches_run_shs_command_line() {
        ...
        let want_win =
            sabrage_core::util::win_path(Some(&bottle.prefix), &ctx.bs_dir.join("Beat Saber.exe"));
        let want_args: Vec<std::ffi::OsString> = ["--bottle", "Steam", "--no-update", "--cx-app"]
            .into_iter()
...
mod run_sh_text_parity {
    ...
    fn native_run_only_die_text_is_verbatim_in_run_sh() {
        ...
        let text = run_sh();
        let scratch =
            std::env::temp_dir().join(format!("sabrage-parity-run-only-{}", std::process::id()));
        std::fs::remove_dir_all(&scratch).ok();
        std::fs::create_dir_all(&scratch).unwrap();
    ...
    fn the_launch_banner_lines_are_verbatim_in_run_sh() {
        ...
        assert!(
            exit_line.starts_with("wine exited with status 0 (log: ") && exit_line.ends_with(')'),
            "{exit_line}"
        );
        assert_verbatim(
            &text,
...
mod lib_sh_contract {
    ...
    fn lib_sh_tap_and_chk_return_zero_when_tap_disabled() {
        let root = repo_root();
        let home =
            std::env::temp_dir().join(format!("sabrage-parity-lib-sh-home-{}", std::process::id()));
