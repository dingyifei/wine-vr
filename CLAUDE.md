...
Caution: ctest runs with an isolated HOME under `build/test-env`; running a test binary directly **clobbers** your real `~/Library/Application Support/OXRSys/oxrsys-runtime.toml` (config-writing suites overwrite it — it has happened). Canonical rule: `ext/oxrsys/docs/testing-and-conformance.md`.
...
- **Three oxrsys build trees coexist:** `ext/oxrsys/build` (arm64, native dev/tests, used by Gate 1 tooling), `ext/oxrsys/build-x64` (x86_64 + ALVR, what the pipeline installs from), and `ext/oxrsys/build-helper-arm64` (the native-arm64 `oxrsys-encoder-helper`, staged next to the build-x64 runtime dylib). Helper arch is CMake-enforced: `OXRSYS_BUILD_ENCODER_HELPER=ON` on any non-thin-arm64 configure is a configure-time FATAL, and a build-x64 re-configure spares a staged arm64 helper while sweeping anything else (pre-2026-08 it deleted the staged helper — if a session streams `(H.264, in-process)`, that used to be why). `doctor` 9b and `run`'s preflight both check presence + arch; `run` self-heals by restaging from `build-helper-arm64`. Don't conflate the trees; all read the same user config.
